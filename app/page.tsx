...
export default function Home() {
  ...
  async function readPaperSheet() {
    ...
    try {
      ...
      setCompany(result.company || ''); setProject(result.project || ''); setCostCenter(result.costCenter || ''); setAfe(result.afe || '')
      setTransactionType(transactionTypes.includes(result.transactionType) ? result.transactionType : '')
      setTakenBy(result.takenBy || ''); setPreparedBy(result.preparedBy || ''); setApprovedBy(result.approvedBy || ''); setNotes(result.notes || '')
      setLines(Array.isArray(result.lines) && result.lines.length ? result.lines : [emptyLine()])
      setStage('review')
    ...
    <main className="shell">
      ...
      {stage === 'capture' ? (
        <section className="card">
          ...
          {file && <div className="row" style={{ marginTop: 12 }}><div className="muted">Use the form-specific reader to map handwriting into these fields.</div><div className="spacer" /><button className="button" type="button" onClick={readPaperSheet} disabled={reading}>{reading ? 'Reading handwriting…' : 'Read handwriting'}</button></div>}
