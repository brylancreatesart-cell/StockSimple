        ...
        <section className="card review-card">
          ...
          <div className="review-layout">
            ...
            <div>
              ...
              {reviewLines.length ? <div className="line review-line review-needed"><div className="row"><strong>Lines requiring review</strong><span className="status warning">{reviewLines.length}</span><div className="spacer" /></div>{reviewLines.map(({ line, index }, reviewIndex) => <div className="line" key={index}><div className="row"><strong>Line {index + 1}</strong><span className="status warning">Check this line</span></div><div className="grid grid2" style={{ marginTop: 8 }}><label>Description<input ref={reviewIndex === 0 ? firstReviewRef : undefined} data-review-input="true" value={line.description} onChange={(e) => updateLine(index, 'description', e.target.value)} onKeyDown={(e) => e.key === 'Enter' && focusNextField(e.currentTarget)} /></label><label>Item #<input data-review-input="true" value={line.itemNumber} onChange={(e) => updateLine(index, 'itemNumber', e.target.value)} onKeyDown={(e) => e.key === 'Enter' && focusNextField(e.currentTarget)} /></label><label>Locator<input data-review-input="true" value={line.location} onChange={(e) => updateLine(index, 'location', e.target.value)} onKeyDown={(e) => e.key === 'Enter' && focusNextField(e.currentTarget)} /></label><label>Qty<input type="number" min="0" step="1" inputMode="numeric" data-review-input="true" value={line.quantity} onChange={(e) => updateLine(index, 'quantity', e.target.value)} onKeyDown={(e) => e.key === 'Enter' && focusNextField(e.currentTarget)} /></label></div></div>)}</div> : <div className="status success" style={{ marginTop: 16 }}>✓ No inventory lines require review. All extracted lines can proceed to approval.</div>}
            </div>
          </div>
