  ...
  async function readPaperSheet() {
    ...
    try {
      ...
      const extractedLines: Line[] = Array.isArray(result.lines) && result.lines.length ? result.lines.map((line: { description?: string; itemNumber?: string; location?: string; quantity?: string; ocrConfidence?: number; needsReview?: boolean }) => ({
        description: line.description || '', itemNumber: line.itemNumber || '', location: line.location || '', quantity: line.quantity || '', confidence: typeof line.ocrConfidence === 'number' ? line.ocrConfidence : undefined, needsReview: Boolean(line.needsReview)
      })) : [emptyLine()]
    ...
    <main className="shell">
      ...
      {stage === 'capture' ? (
        <section className="card">
          <h2>New stock record</h2><div className="grid grid2">
            <label>Company<input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Company name" /></label><label>Project<input value={project} onChange={(e) => setProject(e.target.value)} placeholder="Project number or name" /></label><label>Cost center<input value={costCenter} onChange={(e) => setCostCenter(e.target.value)} /></label><label>AFE<input value={afe} onChange={(e) => setAfe(e.target.value)} /></label><label>Transaction type<select value={transactionType} onChange={(e) => setTransactionType(e.target.value)}>{transactionTypes.map((type) => <option key={type} value={type}>{type || 'Select transaction type'}</option>)}</select></label><label>Taken by / received-returned by<input required value={takenBy} onChange={(e) => setTakenBy(e.target.value)} placeholder="Employee name" /></label><label>Prepared by<input value={preparedBy} onChange={(e) => setPreparedBy(e.target.value)} /></label><label>Approved by<input value={approvedBy} onChange={(e) => setApprovedBy(e.target.value)} /></label><label>Remarks / notes<textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Anything unusual about this stock sheet" /></label>
          </div>
        ...
        <section className="card review-card">
          <div className="row"><div><h2 style={{ marginBottom: 4 }}>Review before approval</h2><div className="muted">Only fields that fail independent verification require attention.</div></div><div className="spacer" /><span className={`status ${totalWarnings ? 'warning' : 'success'}`}>{totalWarnings ? `${totalWarnings} item${totalWarnings === 1 ? '' : 's'} need attention` : 'Ready for approval'}</span></div>
          <div className="review-layout"><div>{preview && <img className="preview review-preview" src={preview} alt="Original stock sheet" />}</div><div>
            {headerNeedsReview && <div className="line review-needed"><div className="row"><strong>Critical header verification</strong><span className="status warning">Check AFE / Project Number</span></div><p className="muted" style={{ marginBottom: 10 }}>The independent readings disagreed on a critical identifier. Verify the AFE and Project Number against the original sheet before approval.</p><div className="grid grid2"><label>Project Number<input data-review-input="true" value={project} onChange={(e) => { setProject(e.target.value); setHeaderNeedsReview(false) }} /></label><label>AFE<input data-review-input="true" value={afe} onChange={(e) => { setAfe(e.target.value); setHeaderNeedsReview(false) }} /></label></div></div>}
